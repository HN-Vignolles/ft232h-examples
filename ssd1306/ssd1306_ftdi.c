...
static void SSD1306_cmd_double(uint8_t cmd1, uint8_t cmd2) {
    unsigned char buf[5];
    buf[0] = mpsse->tx; //Opcode 0x10
    buf[1] = 1; //(LengthL)Length=1 means 2 bytes
    buf[2] = 0; //(LengthH)
    buf[3] = cmd1;
    buf[4] = cmd2;
	// Deassert DC pin -> command transmit
...
void SSD1306_Flush(void) {
    ...
    SPIx_SendBuf(mpsse,(uint8_t *)SSD1306_SET_ADDR_0x0,sizeof(SSD1306_SET_ADDR_0x0));

	// Assert DC pin -> data transfer
    PinHigh(mpsse,SSD1306_DC);

	// Transmit video buffer to LCD
    SPIx_SendBuf(mpsse,vRAM,(SCR_W * SCR_H) >> 3);
